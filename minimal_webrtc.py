...
if 'Safari' in user_agent and 'Chrome' not in user_agent:
    st.warning("⚠ Safari has known issues with WebRTC. Please try Chrome or Firefox for better results.")
...
try:
    ...
    tab1, tab2, tab3 = st.tabs(["🎤 Voice Recognition", "🔊 Simple Recorder", "⌨ Text Input"])
    ...
    with tab1:
        ...
        If nothing happens, try the Simple Recorder in the next tab.
        """)
        ...
        if st.button("📝 ANALYZE", key="webrtc_analyze"):
            with st.spinner("Processing audio..."):
                ...
                if len(st.session_state.audio_frames) > 0:
                    ...
                    st.warning("⚠ No audio captured yet. Try speaking into the mic.")
